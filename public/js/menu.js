// time based menu
export function getDynamicMenu(dynamicText) {
    const now = new Date();
    const hour = now.getHours();
    const minute = now.getMinutes();
    const currentMintues = hour * 60 + minute;
    const messages = [
        {
            start: 6 * 60 + 59,
            end: 10 * 60 + 15,
    ...
    let message = messages.find(({ start, end }) => currentMintues >= start && currentMintues <= end);
    // Find the next available message (in future)
    if (!message) {
        let nextAvailableSlot = messages.find(({ start }) => currentMintues < start);
        let timeUntilNextSlot;
