import { getDynamicMenu } from "./menu.js";
document.addEventListener("DOMContentLoaded", () => {
    const body = document.body;
    const themeToggle = document.getElementById("theme-toggle");
    const dynamicText = document.getElementById("dynamic-menu");
    const addToCartButtons = document.querySelectorAll(".add-to-cart");
    const tableBody = document.querySelector("tbody");
    const yourPlateLink = document.querySelectorAll(".your-plate-link");
    ...
    let displayYourOrderLink = sessionStorage.getItem("yourOrderLink");
    if (displayYourOrderLink) {
        yourPlateLink.forEach((link) => (link.style.display = "block"));
    }
    let existingCartItems = JSON.parse(sessionStorage.getItem("cartItems") || "[]");
    if (existingCartItems) {
        Object.values(existingCartItems).forEach((item, index) => {
            if (item.quantity > 0) {
    ...
    function checkActiveLink() {
        ...
        if (currentActive) {
            const activeLink = document.querySelector(`.nav-link[href="#${currentActive}"]`);
            if (activeLink) {
